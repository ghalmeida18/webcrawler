...
con = pymysql.connect(host = '127.0.0.1',
                      	      user = '', #insira aqui o seu usuario, é root?
                              passwd='',#insira aqui a senha so seu banco
                              db = 'Prefeitura',
...
def home(request):
	with con.cursor() as cursor:
			...
			cursor.execute("SELECT Elemento AS divisao, SUM(ValorPagamento) soma FROM Empenho NATURAL JOIN Pagamento Group By Elemento Order By soma")
			elemento = cursor.fetchall()


			cursor.execute("SELECT Funcao AS divisao, SUM(ValorPagamento) soma FROM Empenho NATURAL JOIN Pagamento Group By Funcao Order By soma")
			funcao = cursor.fetchall()

			cursor.execute("SELECT SubFuncao AS divisao, SUM(ValorPagamento) soma FROM Empenho NATURAL JOIN Pagamento Group By SubFuncao Order By soma")
			subfuncao = cursor.fetchall()

			cursor.execute("SELECT Projeto AS divisao, SUM(ValorPagamento) soma FROM Empenho NATURAL JOIN Pagamento Group By Projeto Order By soma")
			projeto = cursor.fetchall()

			cursor.execute("SELECT Destinacao AS divisao, SUM(ValorPagamento) soma FROM Empenho NATURAL JOIN Pagamento Group By Destinacao Order By soma")
			destinacao = cursor.fetchall()

			cursor.execute("SELECT MONTH(DataPagamento) as divisao, Sum(ValorPagamento) as soma From Pagamento GROUP BY MONTH(DataPagamento);")
			...
			cursor.execute("SELECT Elemento, SUM(ValorPagamento) soma From Empenho NATURAL JOIN Pagamento WHere Elemento = '3.1.90.11.07 - Subsidio Do Prefeito';")
			...
			cursor.execute("SELECT Nome, Cargo  FROM Favorecido NATURAL JOIN Empenho WHERE Elemento = '3.3.90.14.05 - Diarias De Demais Servidores' GROUP BY Nome;")
